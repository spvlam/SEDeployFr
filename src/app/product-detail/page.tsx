...
const ProductDetailPage = function(){
  ...
  const id= searchParams.get(PARAMETER_USER.id) ;
  useEffect(() => {
    if (id) {
      console.log(id)
      fetchProductData(id);
  ...
  const fetchProductData = async (id:string) => {
    try {
      ...
      const res = await fetch(API_SERVER_CONFIG.product_detail+id);
 ...
 // Accessing properties of productData
const sizes = productData?.sizes;
const variants = productData?.variants;
const status = productData?.status;
const allOfSizes = productData?.allOfSizes;
const image = productData?.image || "";
  ...
  //
   async function notifyAddTocart() {
    ...
    return (
      <div className="">
        ...
        <h2 className="text-2xl font-semibold flex items-center">
          ...
          <span className="ml-1.5"> {productData?.rating} · {productData?.numberOfReviews} Reviews</span>
        </h2>
        ...
        <div className="mt-10">
          ...
          >
            Show me all {productData?.numberOfReviews} reviews
          </ButtonSecondary>
        </div>
      </div>
    );
            ...
            <div className="relative">
              <div className="aspect-w-16 aspect-h-16 relative">
                <Image
                  ...
                  src={productData?.image || LIST_IMAGES_DEMO[1]}
