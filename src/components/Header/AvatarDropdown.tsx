...
import { LOCAL_STORAGE, PARAMETER_USER } from "@/config/API_config";
export default function AvatarDropdown() {

  let users = new userAction()
  let [user_name, SetUserName] = useState<String | null>('')
  let [urlAvatar, setUrlAvatar] = useState<string | undefined>('https://avatar.oxro.io/avatar.svg?name=LAMNV');
  useEffect(()=>{
    let userNam = window.localStorage.getItem(LOCAL_STORAGE.userName)
    SetUserName(userNam) 
    setUrlAvatar(PARAMETER_USER.ava+userNam)
                ...
                <div className="overflow-hidden rounded-3xl shadow-lg ring-1 ring-black ring-opacity-5">
                  <div className="relative grid grid-cols-1 gap-6 bg-white dark:bg-neutral-800 py-7 px-6">
                    <div className="flex items-center space-x-3">
                      <Avatar imgUrl={urlAvatar}  sizeClass="w-12 h-12" />

                      <div className="flex-grow">
                        <h4 className="font-semibold">{user_name}</h4>
                        <p className="text-xs mt-0.5">THANH XUAN HA NOI</p>
                      </div>
                    </div>
                    ...
                    >
                      ...
                      <div className="ml-4">
                        <p onClick={users.logout} className="text-sm font-medium ">{"Log out"}</p>
                      </div>
                    </Link>
                  </div>
                </div>
