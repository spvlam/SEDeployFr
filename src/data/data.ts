...
export const SPORT_PRODUCTS2: Product[] = [
  {
    id: 1,
 ...
 export let SPORT_PRODUCTS: Product[] = []

export let fetchData = async (url: URL) => {
  try {
...
fetchData(new URL(API_SERVER_CONFIG.product_list)).then(result => {
  fetchData(new URL(API_SERVER_CONFIG.product_varients)).then(dat => {
    PRODUCTS = result.map((item: Product) => {
      item['variants'] = dat
      return item
    })
...
export let fetchProduct = async (url: URL , url2 : URL)=>{
  try{
      let [result, dat] = await Promise.all([fetchData(url),fetchData(url2)])
      let data = result.map((item: Product) => {
        item['variants'] = dat;
        return item;
      });
      SPORT_PRODUCTS = PRODUCTS = data;
      return data;
  }catch(err){
     console.log(err)
     return []
